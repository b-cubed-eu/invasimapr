...
fab_data = get_occurrence_data(data = '/data/gbif_fabaceae_0038785-241126133.csv',
                                source_type = 'local_csv',
                         # options: 'local_csv', 'data_frame', 'gbif')
                                sep = '\t', # Default: ','
...
•	site_xy: Holds spatial coordinates of sampled sites (Table 1).
•	site_sp: Site-by-species matrix for biodiversity assessments (Table 2).
•	sp_trait: Species-specific trait data essential for understanding how species traits influence interactions (Table 3).
•	site_env: Site-specific environmental data provides contextual information about the conditions at each study location (Table 4).
...
grid_list = generate_grid(data = fab_format$site_sp,
                          ...
                          grid_size = 0.25,    # Grid size in degrees
                          sum_col_range = 4:ncol(data),  # Columns to summarize
...
Interaction Kernel: The Gaussian-like kernel function is used to compute pairwise interaction strengths based on trait distances, where smaller trait distances lead to stronger interactions: r(): Growth rates for alien species in various environments. alpha(): A matrix of interaction strengths, with higher values indicating stronger competitive effects when species’ traits are similar.
r(alien_x, enviro) is a placeholder function returning a constant growth rate (here fixed at 1). In practice, this would be replaced by a more complex function capturing how an alien species’ intrinsic growth rate depends on both its traits (alien_x) and the environment (enviro).
...
compute_invasibility_noLoop = function(i, sbt, env, sbs, inv) {
  ...
  reshuffle_results = future_lapply(1:s_spp, function(species_index) {
...
num_cores = detectCores() – 1 # keeps one system core free
