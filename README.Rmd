...
![](reference/figures/invasimapr_flowchart.png){width=80%}
...
1. **Intrinsic growth** $r_{is}$: the abiotic match of species $i$ to site $s$.
2. **Competitive pressure** \(C^{\mathrm{(raw)}}_{is}\): suppression exerted by residents.
3. **Interaction tensor** \(I_{ijs}\): trait overlap): pairwise impact combining trait similarity, environmental suitability, and resident abundance.
...
In this way, similarity is translated into a competition coefficient with a Gaussian kernel. Here <a id="def-dij"></a>$d_{ij}$ is trait dissimilarity (0 = identical, 1 = very different) and <a id="def-sigmat"></a>$\sigma_t$ controls how quickly competition fades as species diverge. The general relation <a id="def-gall"></a>$g^{\mathrm{(all)}}_{ij}$ is computed over the full species set to provide consistent distances.   
...
Thus the suppression of invader $j$ at site $s$ will increase when:
1. Species $i$ and $j$ are very similar = large $\alpha_{ij}$
2. Residents $j$ are well-matched with environment $\mathbf{E}_s$ = large $K_e$
3. Residents $j$ are very common/thriving = large $N^{*}_{js}$
...
```{r plot-overview, fig.width = 12, fig.height = 6.5, echo=FALSE}
# Create boxes
box_layer = function(xmin, xmax, ymin, ymax, label, fill = "white", size = 3.8) {
  list(
    ggplot2::geom_rect(aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax),
              fill = fill, color = "grey20", linewidth = 0.6),
    ggplot2::annotate("text",
             x = (xmin + xmax)/2, y = (ymin + ymax)/2,
             label = label, size = size, lineheight = 1.12)
  )
}

# convenience: left-aligned text inside a small box (for the 3 badges)
badge = function(xmin, xmax, ymin, ymax, label_expr, size = 3.2) {
  list(
    ggplot2::geom_rect(aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax),
              fill = "#fff9e6", color = "grey50", linewidth = 0.4),
    ggplot2::annotate("text",
             x = xmin + 0.15, y = (ymin + ymax)/2, hjust = 0,
             label = label_expr, parse = TRUE,
             size = size, lineheight = 1.05)
  )
}

# Make data.frame
df_blank = data.frame(x = 0, y = 0)

# Plot
p = ggplot(df_blank, aes(x, y)) +
  # Left column
  box_layer(-9.8, -5.2,  2.2,  5.0, "Traits +\nEnvironment\n(abiotic drivers)") +
  box_layer(-9.8, -5.2, -3.8, -0.6, "Resident community\n(traits, performance,\nabundance)") +

  # Middle-left components
  box_layer(-3.9, -0.2,  2.6,  4.4,
            expression("Intrinsic growth\npotential  " ~ (r[is])),
            fill = "#eaf6ea") +
  box_layer(-3.9, -0.2, -3.0, -1.1,
            expression("Competitive\npressure  " ~ (C[is])),
            fill = "#fbe9e9") +

  # Center
  box_layer( 0.9,  4.7, -0.6,  1.8,
            expression("Invasion\nfitness" ~ (lambda[is])),
            fill = "#eef2fb") +

  # Right outputs
  box_layer( 6.2,  9.5,  1.9,  3.9,
            expression("Site\ninvasibility" ~ (V[s]))) +
  box_layer( 6.2,  9.5, -3.1, -1.1,
            expression("Species\ninvasiveness" ~ (V[i]))) +

  # Three badges – now wider, spaced, and left-aligned
  badge(-9.8, -5.2, -4.95, -4.20,
        "paste('Trait similarity  ', alpha[ij],'  from  ', d[ij])") +
  badge(-9.8, -5.2, -5.85, -5.10,
        "paste('Resident env. match  ', K[e],'(',Delta[js],')')") +
  badge(-9.8, -5.2, -6.75, -6.00,
        "paste('Resident abundance  ', N[js]^'*')") +

  # Arrows (minor tweaks to avoid overlaps)
  ggplot2::geom_curve(aes(x = -5.2, y = 3.55, xend = -3.9, yend = 3.35),
             curvature = 0.05, arrow = grid::arrow(length = grid::unit(3, "mm")), linewidth = 0.5) +
  ggplot2::geom_curve(aes(x = -5.2, y = -2.05, xend = -3.9, yend = -2.05),
             curvature = -0.03, arrow = grid::arrow(length = grid::unit(3, "mm")), linewidth = 0.5) +
  ggplot2::geom_curve(aes(x = -5.2, y = -4.58, xend = -3.9, yend = -1.45),
             curvature = -0.18, arrow = grid::arrow(length = grid::unit(2.5, "mm")), linewidth = 0.45) +
  ggplot2::geom_curve(aes(x = -5.2, y = -5.48, xend = -3.9, yend = -1.68),
             curvature = -0.06, arrow = grid::arrow(length = grid::unit(2.5, "mm")), linewidth = 0.45) +
  ggplot2::geom_curve(aes(x = -5.2, y = -6.38, xend = -3.9, yend = -1.91),
             curvature = 0.10, arrow = grid::arrow(length = grid::unit(2.5, "mm")), linewidth = 0.45) +

  # r_is & C_is -> lambda_is
  ggplot2::geom_curve(aes(x = -0.2, y = 3.05, xend = 0.9, yend = 0.94),
             curvature = -0.22, arrow = grid::arrow(length = grid::unit(3, "mm")),
             linewidth = 0.6, color = "#2c7a2c") +
  ggplot2::geom_curve(aes(x = -0.2, y = -2.05, xend = 0.9, yend = 0.10),
             curvature = 0.22, arrow = grid::arrow(length = grid::unit(3, "mm")),
             linewidth = 0.6, color = "#b22222") +

  # lambda_is -> Vs & -Vi
  ggplot2::geom_curve(aes(x = 4.7, y = 0.90, xend = 6.2, yend = 2.70),
             curvature = -0.25, arrow = grid::arrow(length = grid::unit(3, "mm")), linewidth = 0.6) +
  ggplot2::geom_curve(aes(x = 4.7, y = 0.00, xend = 6.2, yend = -2.00),
             curvature = 0.25, arrow = grid::arrow(length = grid::unit(3, "mm")), linewidth = 0.6) +

  # Compact equation note
  ggplot2::annotate("text", x = -7.6, y = -8.15, size = 3.1, color = "grey25",
           label = "I[ijs] == alpha[ij] %.% K[e](Delta[js]) %.% N[js]^'*'",
           parse = TRUE) +

  ggplot2::coord_equal(xlim = c(-10, 10), ylim = c(-9, 6), expand = FALSE) +
  ggplot2::theme_void() +
...
# Optional: save as high-res PNG (good for Word)
# ggsave("fig_invasimapr_schematic.png", p, width = 9, height = 5.2, dpi = 300)
```
...
Aggregate the fitness matrix to site-level **invasibility** [\(V_s\)](#def-Vs) and species-level **invasiveness** [\(V_i\)](#def-Vi). Map \(\lambda_{is}\), \(V_s\), and \(V_i\), explore uncertainty and sensitivity, and create concise products for management and reporting.

> **Note.** Different summaries can be used. For example, [\(V_s\)](#def-Vs) can be the mean of [\(\lambda_{is}\)](#def-lambda) over species \(i\); and [\(V_i\)](#def-Vi) can be the mean across sites or a sum over positive values only, depending on management goals.
...
> **Figure 3**: Spatial distribution of site-level species richness across South Africa, derived from a species-by-abundance matrix. Each grid cell indicates the number of species observed (numeric labels), with colours representing the square-root–transformed richness (√richness) to emphasize variation across the landscape. Warmer colours denote higher richness, while cooler colours denote lower richness. National boundaries are shown for geographic reference.
...
> Figure 5: Hierarchical clustering of species based on functional traits using Gower dissimilarity. The dendrogram partitions species into four distinct functional groups (coloured branches), reflecting shared trait combinations that may underlie ecological similarity.
...
> **Figure 6**: Principal Coordinates Analysis (PCoA) of species in reduced-dimensional trait space, based on Gower dissimilarity. Kernel density contours highlight regions of high species density (hotspots of trait similarity) and illustrate the overall structure of the functional trait space.
...
* **Per-Trait Similarity Table**: `out$similarity` holds percentage similarity (0-100) for each trait column, computed as either i) *Numeric traits* as the scaled mean pairwise similarity; or -Vi) *Categorical traits* as the proportion of identical pairs.
...
> **Figure 9**: Combined functional trait space workflow. Outputs from the compute_trait_space() function, which integrates multiple trait-based analyses into a unified framework. Shown are: (top left) hierarchical clustering of species by Gower trait distance; (top right) community-level functional diversity metrics (FDis, FRic, RaoQ); (bottom left) species centrality histogram (distance to centroid); and (bottom right) trait space ordination (PCoA with kernel density). Together, these visualisations capture trait similarity, trait-space structure, species’ functional positions, and overall community-level functional diversity.
...
* `d_ij` and/or `g_all` — trait dissimilarity matrix (residents + invaders) [$g^{\mathrm{(all)}}_{ij}$](#def-gall)
* `Nstar` — resident × site abundance matrix from `pred$predictions` [$N^{*}_{js}$](#def-Nstar)
...
In plots, bar height represents cumulative [$\lambda_{is}$](#def-lambda) (summed over sites), ordered from highest to lowest [$V_i$](#def-Vi).
...
* **Proportion of invaders with positive fitness** — fraction of invaders with [$\lambda_{is} > 0$](#def-lambda).
  *This metric only has ecological meaning if some [$\lambda_{is}$](#def-lambda) can be negative (i.e. invaders that fail to establish). If all values are positive, the proportion saturates at 1 and becomes redundant.*
...
We identify **clusters of sites and invaders** with similar invasion fitness patterns using hierarchical clustering of [$\lambda_{is}$](#def-lambda) from `lambda_mat`. This reveals ecological profiles or invasion signatures — types of sites most at risk, or types of invaders most threatening — enabling targeted surveillance and intervention.
...
Mapping these categories provides a spatial overview of invasibility profiles, highlighting geographic concentrations of risk.
...
* **Categorical traits** — mean [\(\lambda_{is}\)](#def-lambda) per trait level identifies functional profiles linked to high or low establishment.
