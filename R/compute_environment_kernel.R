...
#'
#' @examples
#' # Suppose: site_env (site_id, x, y, env1:env10) and a long prediction table pred$predictions
#' ek = compute_environmental_kernel(
#'   site_env        = site_env,
#'   predictions     = pred$predictions,   # columns: species, site_id, pred
#'   site_col        = "site_id",
#'   method          = "gower",
#'   kernel          = "gaussian",         # similarity
#'   sigma_method    = "sd"
#' )
#' str(ek$env_opt); dim(ek$env_dist); dim(ek$K_env)
#'
...
compute_environment_kernel = function(
  ...
  if (is.null(abundance_wide)) {
    ...
    if (requireNamespace("tidyr", quietly = TRUE)) {
      ...
      abundance_wide = matrix(NA_real_, nrow = length(si), ncol = length(sp),
                               dimnames = list(si, sp))
  ...
  if (method == "gower") {
    ...
    if (!all(num_only)) {
      warning("Non-numeric env columns dropped for method = '", method, "': ",
